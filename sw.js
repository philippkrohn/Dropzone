const VERSION='dropzone-1.0.0';
const ROOT=new URL('./',self.location.href);
const CACHE=VERSION+':'+ROOT.pathname;
const PAGES=[18,20,22,23,30,32,34,35,39,41,43,44,45,47,48,49,50,52,53];
const CORE=['./','index.html','style.css','app.mjs','core.mjs','favicon.svg','data/cases.json','data/missions.json','data/reference.json','data/words.json','data/chapters.json'];
...
self.addEventListener('message',e=>{if(e.data?.type!=='CACHE_ALL')return;const port=e.ports[0];e.waitUntil((async()=>{try{const c=await caches.open(CACHE);let count=0;for(const url of ALL){const r=await fetch(url,{cache:'reload'});if(!r.ok)throw new Error('Mindestens eine Datei konnte nicht geladen werden.');await c.put(url,r);port.postMessage({count:++count,total:ALL.length});}port.postMessage({done:true});}catch(err){port.postMessage({done:true,error:err.message});}})());});
