...
EXPECTED='63f0cf70824be2dd82b668b594a90a4c9f984d58a778e5a076c174887f396f58'
RUN=os.environ['GITHUB_RUN_ID'];ATTEMPT=os.environ['GITHUB_RUN_ATTEMPT']
branch='update/training-catalog-2026-09';repository='philippkrohn/Dropzone'
...
for _ in range(120):
 ...
 u=f'https://api.github.com/repos/{repository}/contents/{path}?ref=update%2Ftraining-catalog-2026-09'
...
def verify(base):
 for _ in range(30):
  try:
   root=public_get(base)
   if hashlib.sha256(root).hexdigest()==hashlib.sha256((ROOT/'netlify-site/public/index.html').read_bytes()).hexdigest():break
  except Exception:pass
  time.sleep(3)
 else:raise RuntimeError('New root is not visible.')
 for file in ['library/catalog.json','library/browser-data.js','quiz/core.mjs','quiz/app.mjs','quiz/data/index.json','quiz/data/bank.json','fraktionen/app.mjs']:
  assert hashlib.sha256(public_get(base+file)).digest()==hashlib.sha256((ROOT/'netlify-site/public'/file).read_bytes()).digest(),file+' differs'
 old=public_get('https://6ab1939ae68f1a942996df56--pkstratagems.netlify.app/doubles/')
 ...
 subprocess.run(['python','tests/browser_v2.py','--base',base,'--public'],cwd=ROOT,check=True)
 return {'publishedFilesMatch':True,'doublesUnchanged':True,'trainingAndCatalogueBrowserTests':True}
result={'bundleSHA256':bundlehash,'previousProductionDeploy':'6ab1939ae68f1a942996df56','productionChanged':False}
...
try:result['productionChecks']=verify(URLS[PROD]);result['passed']=True
 ...
 with zipfile.ZipFile(buf,'w',zipfile.ZIP_DEFLATED) as z:
  ...
  z.writestr('public/index.html',public_get('https://6ab1939ae68f1a942996df56--pkstratagems.netlify.app/'))
  z.writestr('public/_redirects','/* https://6ab1939ae68f1a942996df56--pkstratagems.netlify.app/:splat 200\n')
